...
public class FileIO {
	...
	static String anotherStr = new String();
	...
	*/ //DISABLED CODE : Enable by adding asterik on the beginning of this line
	public static void main (String[] args) {
		...
		try
		{
			...
			readFromFile(args[0]); //read file
			writeToFile(args[1]);//write file

		}
		catch (Exception e)
		{

			System.out.println("Read File or Write File Missing \nDoesn't exist "+e.getMessage());
	...
	*/
		...
		java.io.File inFile=new java.io.File(aFile);
			...
			if(inFile.exists() && inFile.canRead())
			{
				...
				while(line != null)
				{
					...
					anotherStr = anotherStr.concat(line+"\n");
					//using another string about for writeToFileMethod
	...
	The outFile is the name of the
	*/
	static public void writeToFile(String aFile)
	...
	throws java.io.IOException
	{
		...
		//outFile = new java.io.File(aFile); //made it a static class variable
			...
			if(!outFile.exists() || outFile.canWrite())
			{
				...
				//* The code is used to read from a source.
				//javax.swing.JTextArea tempAreaText = new javax.swing.JTextArea();
				//bufferedOutStream.write(informationArea.getText()); //works from example
				...
				//bufferedOutStream.write(anotherStr); //works? reads from a text object?
				//bufferedOutStream.write(tempAreaText.getText()); //works
				//*/
